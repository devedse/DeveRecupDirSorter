...
$7zFilePath = Join-Path $outputDir "DeveRecupDirSorter.7z"
$zipFilePath = Join-Path $outputDir "DeveRecupDirSorter.zip"

DeleteFileIfExists $7zFilePath
DeleteFileIfExists $zipFilePath
DeleteFolderIfExists $outputDir

$buildPath = Join-Path $solutionRoot "DeveRecupDirSorter\bin\Release\netstandard2.0"
